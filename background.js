...
(() => {
  ...
  var E = class {
    constructor(t, s, e2, a2 = {}) {
      ...
      l(this, "attempts", 0);
      this.url = t, this.opts = { model: e2.model, method: "POST", stream: e2.stream, reconnect: { maxAttempts: 5, delayMs: 1e3, backoff: true }, headers: { Authorization: s, "Content-Type": "application/json" }, body: { messages: e2.messages } }, this.callbacks = a2;
    }
    async wait(t) {
    ...
    async start() {
      ...
      for (; this.running; ) try {
        ...
        if ((e2 = (s = this.callbacks).onError) == null || e2.call(s, o), !this.running) break;
    ...
    async openOnce() {
      ...
      if (this.opts.body != null) {
        ...
        y(this.opts.headers) ? e2 = JSON.stringify(this.opts.body) : typeof this.opts.body == "string" || this.opts.body instanceof FormData ? e2 = this.opts.body : (s.set("Content-Type", "application/json"), e2 = JSON.stringify(this.opts.body));
      }
      const a2 = await fetch(this.url, { method: this.opts.method, headers: s, body: e2, signal: this.controller.signal });
      if (!a2.ok) {
        const r = await b(a2);
        throw new Error(`SSE POST failed: ${a2.status} ${a2.statusText} - ${r}`);
      }
      if ((o = (c = this.callbacks).onOpen) == null || o.call(c, a2), !this.opts.stream) {
...
`, c = (o) => {
    ...
    parseEvent(t) {
      const s = t.split(/\r?\n/);
      let e2 = [], a2;
      for (const o of s) o.startsWith("id:") ? a2 = o.slice(3).trim() : o.startsWith("data:") && e2.push(o.slice(5).replace(/^\s?/, ""));
...
`);
  ...
  var C = class {
    constructor(t, s, e2) {
      ...
      l(this, "template");
      l(this, "stream");
      this.model = t, this.template = s, this.stream = e2;
    }
    getPrompt(t) {
  ...
  function d(n, t) {
    return JSON.stringify(n, null, 2).replace(/\$\{target_language\}/g, t);
  }
  function w(n, t) {
    const s = [g.identity, "RULES:", ...g.instructions.map((e2) => (e2 = e2.replace("${single_word_output_template}", d(T, n)), e2 = e2.replace("${sentence_or_phrase_output_template}", d(x, n)), e2 = e2.replace("${target_language}", n), e2))];
    return t && (s.push("CONTEXT:"), s.push(t)), s.join(` 
`);
  }
  function _(n) {
    return S.replace("${content_to_translate}", n);
  }
  var v = class {
    constructor(t) {
