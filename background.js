...
`);
  ...
  function w(n, t) {
    const s = [g.identity, "RULES:", ...g.instructions.map((e2) => (e2 = e2.replace("${single_word_output_template}", d(T, n)), e2 = e2.replace("${sentence_or_phrase_output_template}", d(S, n)), e2 = e2.replace("${target_language}", n), e2))];
    return t && (s.push("CONTEXT:"), s.push(t)), s.join(` 
`);
  }
  function _(n) {
    return x.replace("${content_to_translate}", n);
  }
  var O = class {
    constructor(t) {
  ...
  chrome.contextMenus.onClicked.addListener((t, l2) => {
    if (t.menuItemId === "addToEasyNote" && t.selectionText) {
      const o = new C("glm-4-flash-250414", new O(a.defaultLanguage.name), false);
      new k("https://open.bigmodel.cn/api/paas/v4/chat/completions", "Bearer f1987d86479f430491bb5210379b814f.Jh99XCkw0kS7tLiN", o.getPrompt(t.selectionText), { onOpen: (e2) => console.log("opened, status", e2.status), onEvent: (e2) => {
