  ...
  var E = class {
    ...
    async start() {
      ...
      for (; this.running; ) try {
        ...
        if ((e = (s = this.callbacks).onError) == null || e.call(s, o), !this.running) break;
...
`, c = (o) => {
      ...
      try {
        for (; ; ) {
          const { value: o, done: u } = await t2.read();
          if (u) break;
    ...
    parseEvent(t2) {
      const s = t2.split(/\r?\n/);
      let e = [], a2;
      for (const o of s) o.startsWith("id:") ? a2 = o.slice(3).trim() : o.startsWith("data:") && e.push(o.slice(5).replace(/^\s?/, ""));
...
`);
  ...
  function w(n2, t2) {
    const s = [g.identity, "RULES:", ...g.instructions.map((e) => (e = e.replace("${single_word_output_template}", d(T, n2)), e = e.replace("${sentence_or_phrase_output_template}", d(x, n2)), e = e.replace("${target_language}", n2), e))];
    return t2 && (s.push("CONTEXT:"), s.push(t2)), s.join(` 
`);
  }
  function _(n2) {
    return S.replace("${content_to_translate}", n2);
  }
  var v = class {
    constructor(t2) {
